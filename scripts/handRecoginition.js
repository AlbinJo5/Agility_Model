...
function onResults(results) {
  ...
  canvasCtx.drawImage(results.image, 0, 0, 300, 150);
    ...
    if (results.multiHandLandmarks) {
      ...
      imageData.forEach((element2) => {
        ...
        if (results.multiHandLandmarks.length === 2) {
          ...
          var A_a =
            (A_x1 - A_x0) * (A_y2 - A_y1) - (A_x2 - A_x1) * (A_y1 - A_y0);
          var A_b =
            (A_x2 - A_x0) * (A_y3 - A_y2) - (A_x3 - A_x2) * (A_y2 - A_y0);
          var A_c =
            (A_x3 - A_x0) * (A_y1 - A_y3) - (A_x1 - A_x3) * (A_y3 - A_y0);

          var B_a =
            (B_x1 - B_x0) * (B_y2 - B_y1) - (B_x2 - B_x1) * (B_y1 - B_y0);
          var B_b =
            (B_x2 - B_x0) * (B_y3 - B_y2) - (B_x3 - B_x2) * (B_y2 - B_y0);
          var B_c =
            (B_x3 - B_x0) * (B_y1 - B_y3) - (B_x1 - B_x3) * (B_y3 - B_y0);
      ...
      scoreElement.innerHTML = SurfaceArea(
        results.multiHandLandmarks[0][0],
        results.multiHandLandmarks[0][5],
        results.multiHandLandmarks[0][17]
      );
      scoreElement2.innerHTML = SurfaceArea(
        results.multiHandLandmarks[1][0],
        results.multiHandLandmarks[1][5],
        results.multiHandLandmarks[1][17]
      );
    }
  } catch (error) {
    console.log(error);
