...
async function evenlyDistributePointsInGrid(gridWidth, gridHeight, numberOfPoints) {
...
const points = await evenlyDistributePointsInGrid(gridWidth + 200, gridHeight - 100, numberOfPoints);

async function generateRandomPoints(points, numberofGreenBalls, numberofRedBalls) {
    var greenBalls = [];
    var redBalls = [];
...
function onResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandedness) {
        for (let index = 0; index < results.multiHandLandmarks.length; index++) {
            ...
            canvasCtx.beginPath();
            canvasCtx.arc(middlePoint.x * canvasElement.width, middlePoint.y * canvasElement.height, 50, 0, 2 * Math.PI);
    ...
    output.redBalls.forEach((point) => {
        ...
        if (results.multiHandLandmarks && results.multiHandedness) {
            for (let index = 0; index < results.multiHandLandmarks.length; index++) {
                ...
                canvasCtx.beginPath();
                canvasCtx.arc(middlePoint.x * canvasElement.width, middlePoint.y * canvasElement.height, 5, 0, 2 * Math.PI);
                canvasCtx.fillStyle = '#0000FF'
                canvasCtx.fill();


                if (Math.sqrt(Math.pow((middlePoint.x * canvasElement.width - (point.x - 20)), 2) + Math.pow((middlePoint.y * canvasElement.height - point.y), 2)) < 50) {
            ...
            for (let index = 0; index < results.multiHandLandmarks.length; index++) {
                ...
                if (Math.sqrt(Math.pow((middlePoint.x * canvasElement.width - (point.x - 20)), 2) + Math.pow((middlePoint.y * canvasElement.height - point.y), 2)) < 50) {
                    ...
                    console.table({
                        ...
                        pointX: point.x,
                        pointY: point.y,
                        distance: Math.sqrt(Math.pow((middlePoint.x * canvasElement.width - (point.x - 20)), 2) + Math.pow((middlePoint.y * canvasElement.height - point.y), 2))
